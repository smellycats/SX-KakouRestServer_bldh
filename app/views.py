...
def kakou_get(id):
	...
	rows = dhkk.get_picrecord_by_id(id)['data']['rows']
    except Exception as e:
	logger.exception(e)
	raise
    if len(rows) == 0:
    ...
    kkdd_tuple = helper_bldh.build_kkdd_by_url(i['carImgUrl'])
    try:
        item = {
	    'id': i['id'],
	    'hphm': hphm_dict.get(i['carNum'], i['carNum']),
	    'jgsj': i['capDate'],
	    'hpys': hpys_tuple[2],
	    'hpys_id': hpys_tuple[0],
	    'hpys_code': hpys_tuple[1],
	    'kkdd': kkdd_tuple[1],
	    'kkdd_id': kkdd_tuple[0],
	    'fxbh': fxbh_tuple[1],
	    'fxbh_code': fxbh_tuple[0],
	    'cdbh': int(i['carWayCode']),
	    'clsd': i['carSpeed'],
	    'hpzl': '00',
	    'kkbh': kkdd_tuple[0],
	    'clbj': 'F',
...
def kakou_list_get():
    ...
    try:
	dhkk = DHKakou(**app.config['BLDH_INI'])
	rows = dhkk.get_picrecord(**args)['data']['rows']
    except Exception as e:
	logger.exception(e)
	raise
    try:
        ...
        cached = cache.get(str(rows[0]['id']))
        hphm_dict = {u'0': '-'}
	items = []
	for i in rows:
            hpys_tuple = helper_bldh.hpzl2hpys(i['carType'])
